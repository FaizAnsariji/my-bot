...
module.exports.run = async ({ event, api, args, Threads }) => {
    ...
    if (custom.length != 0) {
        ...
        for (const idThread of allThread) {
            ...
            api.changeNickname(`[ ${(threadSetting.hasOwnProperty("PREFIX")) ? threadSetting.PREFIX : global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "FAIZ ANSARI" : global.config.BOTNAME}`, idThread.threadID, idBot, (err) => (err) ? threadError.push(idThread.threadID) : '');
            count+=1;
            await new Promise(resolve => setTimeout(resolve, 500));
        }
