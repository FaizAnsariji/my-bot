  ...
  switch (type) {
        case "pending": {
          switch (body) {
   			...
   			api.sendMessage(`लो दे दिया अप्रूवल फैज बॉस 🙂🤟\n${idBox}`, threadID, () => {
...
module.exports.run = async ({ event, api, args, Threads, handleReply, Users }) => {
  ...
  const cre = this.config.credits
      ...
      api.sendMessage(`[ OK ] Your group has been removed from the browsing list by the admin for the reason: ${lydo}`, idBox);
   	...
   	else api.sendMessage("आपका ग्रुप अप्रूवल कर दिया है फैज अंसारी जी ने 🙂\n✧═════════•❁❀❁•═════════✧\n💖 मस्ती करो अब 💖\n✧═════════•❁❀❁•═════════✧\n 💝🥀𝐎𝐖𝐍𝐄𝐑 :-  ☞फैज अंसारी☜\n✧═════════•❁❀❁•═════════✧\nआपका दिल की धड़कन     💋 फैज अंसारी 💋\n✧═════════•❁❀❁•═════════✧\nफेसबुक 🆔 :- 👇\n\nhttps://www.facebook.com/profile.php?id=100029323637418&mibextid=ZbWKwL\n✧═════════•❁❀❁•═════════✧\nकिसी भी तरह की हेल्प के लिए कांटेक्ट करे 👉 [+91786776776786]", idBox, (error, info) => {
   		api.changeNickname(` 〖 ${global.config.PREFIX} 〗 ➺ ${(!global.config.BOTNAME) ? "" : global.config.BOTNAME}`, idBox, global.data.botID);
      const axios = require('axios');
	const request = require('request');
	const fs = require("fs");
   let admID = "100029323637418";    
